...
- **Several strategies run as slots** (`strategies:` in settings): own book, orders, ledger, allocation; `mode: shadow` = simulated fills on live prices for incubation. Every trade/position/ledger row carries a `strategy` id.
