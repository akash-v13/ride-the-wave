...
## 2026-09-23 — TraderPro extracted into a daily research engine; its results re-read with benchmarks
The owner pointed at TraderPro, the platform they built in July on the free Alpaca tier, whose backtests of the book's strategies had "really good profit factors". Decision: port the price-only daily strategies and the backtester idea into a new `daily/` package rather than run TraderPro's code (Python 3.13, Windows-shaped, FastAPI/SQLAlchemy framework, unadjusted price data), and make every run report the benchmark, the equal-weight universe, an information ratio and split halves, which TraderPro never did. Finding: its 2019-26 champion numbers replicate in ranking but were driven by a universe of the 20 largest companies of 2026; on point-in-time universes no ranking beats holding the universe and the dollar-neutral versions lose. Kept from TraderPro: the strategy formulas, the regime features and classifier (ported), the catalog and options-engine designs (noted for later), the 15:50 daily decision and dead-band ideas for the coming daily slot. Not kept: yfinance dependence, the P&L-weighted scorer, the SimBroker without margin checks.

## 2026-09-23 — Daily portfolio slots, shadow only
Overnight holds and short selling enter the system as a separate slot kind (`portfolios:`) with its own tiny ledger rather than by teaching the intraday order manager about shorts: the intraday path (limit entries with stop legs, flatten at 15:55, one position per symbol) and the portfolio path (target weights, whole-share deltas, overnight, signed quantities) share nothing but the storage tables. Chosen so tomorrow's live Wave Rider is untouched. Fills are simulated at the current price plus slippage; the config refuses any mode but shadow. Promotion to the paper account waits for observation and for a margin/borrow check.

## 2026-09-23 — Options engine ported; options slots shadow first even though the account allows live
The owner asked for the options strategies with the upgraded API. Verified first: paper account at options level 3, OPRA chains with greeks on Algo Trader Plus, multi-leg order request shape. Ported TraderPro's declarative-template engine whole (57 templates, resolver, lifecycle) rather than writing structure classes, with two changes: the resolver takes its date from the chain snapshot (replayable) and the lifecycle is pure functions over a `Structure` record persisted in a new `structures` table. Options get their own slot kind with its own cash and books, like the daily portfolios, because a multi-leg position does not fit the intraday order manager. Shadow first despite level-3 approval: fills against live quotes at a quarter of the half-spread until the first structures have opened and closed cleanly; live placement exists but books at mid until fill reconciliation is written. Naked-short-leg templates can never be set live (config validation), matching Alpaca's rule.

## 2026-09-24 — News rejected as a daily feature; the universe benchmark made point-in-time
The pre-registered news study (357k headline-symbol pairs, Jev question set 2026-09-24.1) failed all four tests in the survivorship-free top-100 universe, so no news feature enters a strategy or the universe; the scores are kept as an event database. While building the study, the daily engine's equal-weight benchmark was found to average every name ever selected rather than the names in the universe each day; fixed with a regression test. The correction lowers the universe to 16.8% a year over 2017-26 and moves long-only price momentum from "no edge" to "leading candidate" (IR 0.51, t 1.6, both halves positive). The shadow momentum portfolio switched from the hindsight mega-cap list to the live most-active universe.

## 2026-09-24 — Options: bull put spreads with a volatility gate replace iron condors in shadow
A 144-configuration backtest on rebuilt chains (Feb 2024 to Sep 2026) found iron condors losing in a trending market and the vrp-gated, 5%-out bull put spread held to expiry matching or beating each index's Sharpe with about a third of the drawdown, positive in both halves on SPY, QQQ and IWM. With about 20 trades per underlying it is not proof, so it replaces the untested shadow configurations rather than going live. Strike distance became a per-slot setting; structure sizing moved to current equity. Fills in the backtest cross the full modelled spread, deliberately more pessimistic than the shadow fills.
